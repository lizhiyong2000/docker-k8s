...
if [[ -d bin/x-pack ]]; then
    ...
    if [[ -n "$ELASTIC_PASSWORD" ]]; then
        [[ -f  "$ES_HOME/config/elasticsearch.keystore" ]] || (run_as_other_user_if_needed elasticsearch-keystore create)
...
if [[ "$(id -u)" == "0" ]]; then
    ...
    if [[ -n "$TAKE_FILE_OWNERSHIP" ]]; then
        chown -R 1000:0 "$ES_HOME/{data,logs}"
    fi
fi

run_as_other_user_if_needed  "$ES_HOME/bin/elasticsearch" "${es_opts[@]}"
